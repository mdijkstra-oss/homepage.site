...
export function sizeLifeCanvas(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, cellSize: number): GolDims & { vw: number; vh: number } {
...
export function paintLife(
  ...
  grid: Uint8Array, prevGrid: Uint8Array, dims: GolDims, cellSize: number,
  hueBuf: Float32Array, lightBuf: Float32Array,
  phase: number, viewport: { vw: number; vh: number }, boosted: boolean,
): void {
  ...
  for (let i = 0; i < N; i++) {
    const cur = grid[i] ? 1 : 0, prev = prevGrid[i] ? 1 : 0;
