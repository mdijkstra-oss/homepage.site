...
export function stepLife(grid: Uint8Array, dims: GolDims): { grid: Uint8Array; alive: number } {
  ...
  const g = grid, n = new Uint8Array(C * R);
  let alive = 0;
  for (let y = 0; y < R; y++) {
    for (let x = 0; x < C; x++) {
      let s = 0;
...
export function spawnFillers(grid: Uint8Array, dims: GolDims, patterns: PatternTable, count: number, rng: () => number = Math.random): Uint8Array {
  return placeRandomFiller(grid, dims, patterns, count, count * 16, rng);
}

export function spawnGliders(
  grid: Uint8Array, dims: GolDims, glider: ReadonlyArray<Cell>,
  snake: ReadonlyArray<{ c: number; r: number }> | null, count: number, rng: () => number = Math.random,
): Uint8Array {
  const { cols: C, rows: R } = dims;
  let next = grid;
  const head = snake?.[0];
...
export function computeColors(
  grid: Uint8Array, dims: GolDims,
): { hueBuf: Float32Array; lightBuf: Float32Array } {
  const { cols: C, rows: R } = dims, N = C * R;
    ...
    while (sp) {
      ...
      for (let dy = -1; dy <= 1; dy++) {
        for (let dx = -1; dx <= 1; dx++) {
          ...
          const ni = ny * C + nx;
          if (grid[ni] && !labelBuf[ni]) { labelBuf[ni] = next; stackBuf[sp++] = ni; }
        }
      }
    }
    sizes[next++] = count;
