...
function isBurstActive(state: BurstState, gameActive: boolean): boolean {
  const hasBurst =
    state.particles.length > 0 || state.bloom !== null || state.screenFlash !== null || state.shocks.length > 0;
...
function stepBurstParticles(particles: readonly Particle[], dt: number, f: number): Particle[] {
  ...
  for (const p of particles) {
    ...
    const vx = p.vx * drag ** f;
    const vy = p.vy * drag ** f + (p.grav ?? 0.28) * f;
...
function drawBurstParticles(ctx: CanvasRenderingContext2D, particles: readonly Particle[], now: number): void {
  for (const p of particles) {
    ...
    ctx.beginPath();
    ctx.arc(p.x, p.y, Math.max(0.4, p.w * 0.75 * (0.4 + p.life)), 0, Math.PI * 2);
    ctx.fill();
  }
}

function stepAndDrawScreenFlash(
